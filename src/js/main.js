...
const commentTemplate = ({
  ...
  return `
    <article id="${id}" class="bg-white relative rounded-md w-full p-3 mb-4 mt-2 grid grid-cols-2 md:grid-cols-[auto_1fr_auto] gap-6 items-start"
      >
      ...
      <div class="col-span-2 md:col-span-1 md:col-start-2">
        ...
        <p class="font-myfont font-medium text-grey-500 mt-2 text-sm/5">${content}</p>
      </div>
...
const replyTemplate = ({
  ...
  const isCurrentUser = username === data.currentUser.username;
  return `
        <div class="flex justify-end border-l-2 pb-2 border-grey-100">
          <article id = "${id}" class="bg-white rounded-md relative w-19/20 p-3 grid grid-cols-2 md:grid-cols-[auto_1fr_auto] gap-6 items-start"
        >
        <div class="col-start-1 row-start-2 w-fit p-2 bg-grey-50 rounded-md flex flex-row gap-3 items-center md:row-start-1 md:col-start-1 md:flex-col">
          <button class="cursor-pointer" onclick="increaseCount(${id})">
            <img 
        ...
        <div class="col-span-2 md:col-span-1 md:col-start-2">
          ...
          <p class="font-myfont font-medium text-grey-500 mt-2 text-sm/5">
            <strong class="text-purple-600">@${replyingTo}</strong> ${content}
          </p>
        </div>
        ...
        ${
          isCurrentUser
            ...
            : `<button class="text-purple-600 self-center group font-medium flex items-center gap-2 justify-self-end cursor-pointer col-start-2 row-start-2 md:absolute md:row-start-1 md:top-0 md:right-8">
          <svg width="14" height="13" xmlns="http://www.w3.org/2000/svg">
            <path
            d="M.227 4.316 5.04.16a.657.657 0 0 1 1.085.497v2.189c4.392.05 7.875.93 7.875 5.093 0 1.68-1.082 3.344-2.279 4.214-.373.272-.905-.07-.767-.51 1.24-3.964-.588-5.017-4.829-5.078v2.404c0 .566-.664.86-1.085.496L.227 5.31a.657.657 0 0 1 0-.993Z"
            fill="#5357B6" class="group-hover:fill-purple-200"/>
